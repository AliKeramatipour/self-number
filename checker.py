...
def add_brick(bricks, word, add_rev = 1, first = -1, last = -1):
    ...
    if word not in bricks or first < bricks[word]:
        print("confirming")
        if verify(word, first, id = 'add_actual_brick'):
            bricks[word] = first
...
def Brick_generator(seq, bricks, n):
    ...
    while l < log_2(l + n + 1):
        ...
        lp = 134 - l - n + 1
        add_brick(bricks, word=tuple(seq[lp:lp+n]), add_rev=0, first=lp)
        lp = 2097172 - lp - n + 1
        add_brick(bricks, word=tuple(seq[lp:lp + n]), first=lp, add_rev=0)
        l += 1
...
def merge_generator(seq, bricks, n):
    ...
    for i in range(n):
        ...
        for word, start in bricks.items():
            print()
            print(f"TRYINT TO MIX {binary(sub_brick)} and {binary(word)} starts at {start}")
...
def pre_computer(seq, n, bricks, mx = 2097172):
    binary_num = 0
    for i in range(n):
        binary_num = (binary_num << 1) | seq[i]
    number = 0
    for i in range(n, len(seq)):
        bin_str = bin(binary_num)[2:].zfill(n)
        binary_tuple = tuple(char == '1' for char in bin_str)
        if binary_tuple not in bricks:
            bricks[binary_tuple] = i - n
        binary_num = ((binary_num << 1) | seq[i]) & ((1 << n) - 1)
